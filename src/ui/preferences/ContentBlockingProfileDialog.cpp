...
void ContentBlockingProfileDialog::save()
{
	...
	else
	{
		...
		ContentBlockingProfile *profile(new ContentBlockingProfile(fileName, m_ui->titleEdit->text(), QLatin1String("adBlock"), url, QDateTime(), QList<QString>(), m_ui->updateIntervalSpinBox->value(), category, (ContentBlockingProfile::HasCustomTitleFlag | ContentBlockingProfile::HasCustomUpdateUrlFlag | ContentBlockingProfile::HasTypeFlag)));
