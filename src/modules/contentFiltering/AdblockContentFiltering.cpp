...
#include "AdblockContentFiltering.h"
...
QList<QChar> AdblockContentFiltering::m_separators(QList<QChar>({QLatin1Char('_'), QLatin1Char('-'), QLatin1Char('.'), QLatin1Char('%')}));
...
void AdblockContentFiltering::parseRuleLine(QString line)
{
	...
	{
		if (ContentFilteringManager::getCosmeticFiltersMode() == ContentFilteringManager::AllFiltersMode)
		{
	...
	{
		if (ContentFilteringManager::getCosmeticFiltersMode() != ContentFilteringManager::NoFiltersMode)
		{
	...
	if (!ContentFilteringManager::areWildcardsEnabled() && line.contains(QLatin1Char('*')))
	{
		return;
	}
...
void AdblockContentFiltering::parseStyleSheetRule(const QStringList &line, QMultiHash<QString, QString> &list)
{
...
void AdblockContentFiltering::addRule(AdBlockRule *rule, const QString &ruleString)
{
	Node *node(m_root);
...
void AdblockContentFiltering::deleteNode(Node *node)
{
...
ContentFilteringManager::CheckResult AdblockContentFiltering::checkUrlSubstring(Node *node, const QString &subString, QString currentRule, NetworkManager::ResourceType resourceType)
{
	ContentFilteringManager::CheckResult result;
	ContentFilteringManager::CheckResult currentResult;
...
ContentFilteringManager::CheckResult AdblockContentFiltering::checkRuleMatch(AdBlockRule *rule, const QString &currentRule, NetworkManager::ResourceType resourceType)
{
	...
	const QStringList requestSubdomainList(ContentFilteringManager::createSubdomainList(m_requestHost));
	...
	if (isBlocked)
	{
		...
		if (rule->isException)
		{
			...
			{
				result.comesticFiltersMode = ContentFilteringManager::NoFiltersMode;
			}
			else if (rule->ruleOptions.testFlag(GenericHideOption))
			{
				result.comesticFiltersMode = ContentFilteringManager::DomainOnlyFiltersMode;
			}
...
ContentFilteringManager::CheckResult AdblockContentFiltering::checkUrl(const QUrl &baseUrl, const QUrl &requestUrl, NetworkManager::ResourceType resourceType)
{
	ContentFilteringManager::CheckResult result;
	...
	{
		ContentFilteringManager::CheckResult currentResult(checkUrlSubstring(m_root, m_requestUrl.right(m_requestUrl.length() - i), QString(), resourceType));
...
ContentFilteringManager::CheckResult AdblockContentFiltering::evaluateRulesInNode(Node *node, const QString &currentRule, NetworkManager::ResourceType resourceType)
{
	ContentFilteringManager::CheckResult result;
	...
	for (int i = 0; i < node->rules.count(); ++i)
	{
		if (node->rules.at(i))
		{
			ContentFilteringManager::CheckResult currentResult(checkRuleMatch(node->rules.at(i), currentRule, resourceType));
...
bool AdblockContentFiltering::parseUpdate(QNetworkReply *reply, QFile &file)
{
